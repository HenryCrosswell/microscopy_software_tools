...
def image_registration_sitk(reference, image_to_register):
    ...
    registration_method.SetInitialTransform(
        sitk.TranslationTransform(reference_sitk.GetDimension())
    )

    # Perform image registration
    final_transform = registration_method.Execute(
        reference_sitk, image_to_register_sitk
    )

    # Apply the transformation to the image
    registered_image_sitk = sitk.Resample(
        image_to_register_sitk, reference_sitk, final_transform, sitk.sitkLinear, 0.0
    )
